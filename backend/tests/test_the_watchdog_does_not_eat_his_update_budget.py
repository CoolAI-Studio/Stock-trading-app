...
信的外部監控。那比這個排程快得多（它實際上好幾個小時才跑一次，#112），而且不吃他任
何額度。
"""
...
def test_it_does_not_ask_more_often_than_the_database_can_sleep():
    """深的那一條平常不查資料庫，但迴圈出事的時候會。

    #100 之後 `?deep=1` 讀的是盯盤迴圈剛跑完一輪的證據，平常一句 SQL 都不送；證據過期
    的時候才自己 `SELECT 1`——也就是迴圈已經出事的那段時間。那段時間裡每一次看門狗都會
    叫醒資料庫一次。

    Neon 免費方案閒置五分鐘才休眠、而且關不掉（額度一個月 400 小時，全月醒著是 730），
    所以間隔要是比休眠門檻密，一次沒人處理的故障就會順便把那顆運算單元釘在醒著的狀態、
    把月額度燒掉——等故障修好，資料庫也停了。不比收盤後的輪詢密，就不會多花。

    （這條原本的理由是「深的那一條每次都查資料庫」，#100 之後不再成立；數字沒變，理由
    換成上面這個。另外量到的是 GitHub 實際上一天只跑六到八次，設定值只是上限——見
    watchdog.yml、#112。）
    """
    import re

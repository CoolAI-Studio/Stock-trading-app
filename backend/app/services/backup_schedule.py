    ...
    for schedule in db.query(BackupSchedule).filter(BackupSchedule.is_enabled.is_(True)).all():
        ...
        try:
            ...
            _send_email(
                ...
                #     它的時機是已經弄丟東西的那一天。
                #
                # 所以現在說的是真的：檔案是完整的、密碼是唯一的鑰匙、而「還原」
                # 目前還不是一顆按鈕（#81）。說不出口的東西就老實說不出口，不要
                # 指向一個不存在的地方。
                (
                    ...
                    "老實說一件事：**app 裡目前還沒有「一鍵還原」**。真的需要把資料"
                    "放回去的時候，會需要一個能對資料庫操作的人幫忙；這個 repo 裡有 "
                    "scripts/inspect_backup.py 可以讀出裡面的內容。\n\n"
                    "先把檔案收好——沒有它的話，那一天連幫忙都幫不上。\n"
                ),

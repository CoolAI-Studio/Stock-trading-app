    ...
    for schedule in db.query(BackupSchedule).filter(BackupSchedule.is_enabled.is_(True)).all():
        ...
        try:
            ...
            _send_email(
                ...
                #
                # 內文原本寫著「要檢視內容：python scripts/inspect_backup.py 這
                # 個檔案」。收信的人是按按鈕部署的——他手上沒有這個 repo、沒有
                # Python、也沒有終端機。而這一封特別諷刺：整個排程備份存在的理
                # 由就是讓他不用記得去做一件手動的事，然後信裡叫他做一件他做不
                # 到的事。
                #
                # 腳本沒有被拿掉（對真的在命令列上的人它是對的），改的是先講他
                # 做得到的那一個。
                (
                    "附件是這個帳號的加密備份。把它存起來就好，不用打開它。\n\n"
                    "用你設定備份時輸入的密碼才能打開；那組密碼沒有存在這封信裡，"
                    "也不會出現在任何郵件中——弄丟就打不開這些檔案了。\n\n"
                    "要還原：到 app 的「帳號」頁，備份那一區有「從備份還原」，"
                    "選這個檔案、輸入那組密碼就好。\n\n"
                    "（想在自己的電腦上先看看內容的話，這個 repo 裡有 "
                    "scripts/inspect_backup.py。不需要，只是給想看的人。）\n"
                ),

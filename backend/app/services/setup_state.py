        ...
        missing.append(
            MissingSetting(
                ...
                how=(
                    ...
                    # 「這個檔案會被清空」講的是狀態；他需要的是**此刻該做什麼**。
                    #
                    # 留空也部署得起來（DATABASE_URL 有預設值），而那條路是好的——他可
                    # 以先看到東西動起來，再決定要不要去註冊一家資料庫。但如果他在這
                    # 個狀態下先建了帳號、寫了策略、設好通知，等資料庫接上去，那些全
                    # 部不見，換成一個全新的空的。畫面從頭到尾都正常，所以他不會預期。
                    # 純文字，不要 markdown：這幾個欄位是 {item.how} 直接渲染
                    # 的，星號會原樣出現在畫面上。first-run 那一關抓到過一次。
                    "在這一格弄好之前，先不要建立帳號或新增策略——"
                    "現在存的東西都在那個會被清空的檔案裡，接上資料庫之後不會跟著過來。"
                ),
                generator=None,

...
async def ws_endpoint(websocket: WebSocket, ticket: str, db: Session = Depends(get_db)) -> None:
    ...
    # **從這裡就要進 try。** 快照那一句會查資料庫，而資料庫這下真的會睡了（#99、
    # #101）——閒置之後的第一次查詢要付一次冷啟動（量到 2.43 秒），那段時間裡連線逾時是
    # 真的會遇到的。它原本在 try 外面，所以一丟例外就留下一筆指向死掉 socket 的登記：
    # 已經 accept、已經登記進 manager，而 finally 裡的 disconnect 收不到。
    try:

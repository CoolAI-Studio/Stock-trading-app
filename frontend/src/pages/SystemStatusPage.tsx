    ...
    <ul className="space-y-1 text-xs">
      {behindDays !== null && behindDays >= 0 && (
        <li className="pb-1 text-amber-200/90">
          你已經落後 {behindDays} 天
          {/* **門檻是天，不是月。**
              原本是 30 天，而那是照「他還沒去按」的模型訂的。#52 之後後端是自己更新
              的：stable 一前進，他的服務幾分鐘內就重新部署。所以「落後 5 天」的意思
              不是他還沒去按，是那條線已經斷了五天——而按 30 天的門檻，他要再等 25 天
              才會看到唯一一句告訴他該去哪裡看的話，而那 25 天的每一則提醒都跑在一個
              沒有拿到安全修補的版本上。
              留三天而不是一天：一次部署平台排隊、一次同步晚一輪都還在裡面，而一個平
              常就在喊的東西，真的出事那一次也不會有人看。 */}
          {behindDays >= 3 && (
            <span className="ml-1">
      ...
      {update?.behind === true && (
        <div className="space-y-2 rounded border border-amber-700 bg-amber-950/40 px-3 py-2 text-sm text-amber-200">
          <p>
            ...
            更新裡可能包含安全修補。
            {/* 「去按一次重新部署」把一件自動的事講成了待辦事項。按鈕部署的那一份追
                stable、自動部署，所以正常情況下他什麼都不用做——講成待辦事項，他會去
                按一次、看到沒變、然後放棄，而真正該做的（看那次 build 的紀錄）沒有人
                說。手動架的那一份確實要自己按，所以兩句都留，順序照可能性排。 */}
            這一份的設計是<strong>自己更新</strong>的（追 <code>stable</code>、自動部署），
            所以正常情況下幾分鐘內就會拿到，你什麼都不用做。一直停在這裡的話，去你部署
            後端的平台按一次重新部署；還是沒變，就去看那次 build 的紀錄。
          </p>
          {/*

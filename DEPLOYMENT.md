...
2. 新增**兩個** HTTP(s) Monitor。兩個做的是不同的事，而**間隔不一樣是刻意的**（理由在
   下一節）：

   | | URL | Monitoring Interval | 它負責什麼 |
   | --- | --- | --- | --- |
   | 保活 | `https://<你的網址>/healthz` | **5 分鐘** | 讓 Render 不要休眠 |
   | 警報 | `https://<你的網址>/healthz?deep=1` | **30 分鐘** | 壞掉的時候寄信給你 |

3. **兩個都要在 Alert Contacts 加上你的 Email**，否則就算後端壞掉也不會通知你。
4. **它顯示 Down 的時候不要按暫停。** 暫停不會修好那件事，只會讓服務重新開始休眠——變成兩個問題。去 app 的系統狀態頁看是哪一格紅的。

> **為什麼不是一個就好（把 5 分鐘那個直接指向 `?deep=1`）？**
>
> 因為那兩個網址的成本不一樣。沒帶參數的那一條**完全不碰資料庫**，所以每 5 分鐘打一次
> 是免費的；`?deep=1` 會去查資料庫，而 Neon 免費方案是**閒置五分鐘才休眠**——每 5 分鐘
> 查一次，等於把那顆運算單元釘在醒著的狀態，一個月要 730 小時，而額度只有 400。
>
> 拆成兩個之後：保活照樣每 5 分鐘（Render 需要的是「有流量」，不是「有查詢」），而資
> 料庫每 30 分鐘才被碰一次——跟收盤後的盯盤輪詢同一個節奏，所以不會多花任何額度。
>
> 代價是壞掉之後最多晚 30 分鐘才收到信。這是刻意選的：晚半小時知道，比每個月後半整個
> 停擺好。
...
題。真的答案在 `?deep=1` 上，而那個一天只被打 288 次（每 5 分鐘一次），那是刻意接受的
代價——它是「有沒有人會被通知」唯一的外部觀測。
...
| `database` | 資料庫連不上（服務掛掉、連線字串失效、免費額度用完） | 不會，而且**沒帶參數時根本不查**（那一次查詢本身就是額度成本），回 `skipped` |

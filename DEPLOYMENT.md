...
> ⚠️ **但「暫停」和「額度用完」是兩件事，而後者不會自動喚醒。**
>
> Neon 免費方案給的是 **100 CU-hours/月**，官方換算是「0.25 CU 的運算單元跑 400 小時」。
> 閒置 5 分鐘就休眠，而且免費方案**關不掉**這個行為——問題是這個 app 讓它幾乎不會閒置：
> 盯盤迴圈開市時每 5 秒、**收盤後也每 5 分鐘**碰一次資料庫，而保活監控每 5 分鐘打一次
> `/healthz`（會 `SELECT 1`）。5 分鐘的節奏對上 5 分鐘的休眠門檻，等於一直醒著。
>
> 一個月 730 小時，額度 400 小時，所以**大約第 17 天就會用完**，然後

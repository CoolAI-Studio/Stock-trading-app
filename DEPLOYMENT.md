...
> ⚠️ **「暫停」和「額度用完」是兩件事，而後者不會自動喚醒。**
>
> Neon 免費方案給的是 **100 CU-hours/月**（官方換算：0.25 CU 的運算單元跑 400 小時），
> 閒置 5 分鐘就休眠，而且免費方案**關不掉**這個行為。用完之後
> 「compute is suspended until the next billing period」——不會自己回來。
>
> **這個 app 是照著那個數字調過的**：收盤後把問價的間隔拉到半小時
> （`market_loop.CLOSED_POLL_INTERVAL_SEC`），讓那顆運算單元有 25 分鐘是睡著的，整個月
> 用得掉的就落在額度以內。所以正常使用不會撞到。
>
> **兩種情況例外，值得先知道：**
>
> 1. **只看加密貨幣。** 24 小時交易 = 市場永遠是開的 = 每 5 秒問一次 = 資料庫永遠不會
>    閒置，額度大概月中就會用完。這種情況用 Supabase（它沒有運算時數的計費，只有
>    「一整週沒人用就暫停」，而這個 app 不會閒置一週）。
> 2. **有人把輪詢間隔調快。** 那一格看起來像效能參數，實際上是這個額度的預算。
>
> 用 Supabase 的話，有**兩個一定要做對的地方**：
>
...
2. 新增一個 **HTTP(s) Monitor**：
   - URL：`https://<你的 Render 後端網址>/healthz?deep=1`（**參數不要漏掉**，理由在下一節）
...
> **這個五分鐘不會吃掉你資料庫的額度。**
>
> 免費方案的資料庫閒置 5 分鐘就休眠，而休眠的時間不算錢——所以「每 5 分鐘查一次資料
> 庫」等於讓它永遠不睡，一個月的額度大概月中就用完。這一版之前確實是這樣。
>
> 現在 `?deep=1` 不會自己去查了：盯盤迴圈每跑完一輪就等於把資料庫整套用過一遍，這個
> 端點直接拿那個當答案（`health._database_answer`），只有在迴圈太久沒有回報成功的時
> 候才自己問一次。所以你要打多密都可以。
>
> 代價寫在這裡：資料庫如果在兩輪之間死掉，這一格最多晚一個輪詢週期才變紅。沒有辦法
> 比這更早——除非讓資料庫永遠不休眠，而那個代價是每個月後半完全沒有提醒。
...
- **Render 的健康檢查**（服務設定裡的 Health Check Path）問的是「要不要重開這台機器」。
  它失敗 60 秒，Render 就會把你的服務重開；部署當中 15 分鐘等不到通過，Render 會直接
  取消那次部署。
- **UptimeRobot** 問的是「要不要寄信給我」。

所以**沒帶參數**的 `/healthz` 只在「重開有機會修好」的時候回 503，而那**只剩一件事**：
盯盤那條迴圈不再往前（行程還活著、還在回應，但卡住了）。連資料庫連不上都不算——撐得過
60 秒的那幾種原因重開一萬次都回不來。

少了這個分別，一顆下架的股票就會讓你的服務每十幾分鐘被重開一次、永遠，而且你從此收不
到任何更新——因為新版永遠等不到一次通過的健康檢查。

**帶了 `?deep=1`** 就是完整的那一條：任何一項失敗都回 503。這是你要給 UptimeRobot 的
那一個。

還有一個差別，而它是**額度問題**不是設計潔癖：**沒帶參數的那一條完全不碰資料庫。**

平台的健康檢查一直在打那個網址（那正是它的工作），而你的 app 只要開在瀏覽器裡，畫面也
每 30 秒打一次。如果每一次都去 `SELECT 1`，那顆資料庫的運算單元就被釘在醒著的狀態——
Neon 閒置五分鐘才休眠，而這些探測比五分鐘密得多。收盤後的輪詢拉長到半小時也救不了，因為
睡著的是盯盤迴圈，不是健康檢查。

所以沒帶參數的那一條回的 `database` 是 `skipped` 而**不是** `ok`：它沒問，就不能說沒問
題。

而 `?deep=1` 那一條也不是每次都自己去問。盯盤迴圈每成功跑完一輪，就等於把資料庫整套讀
寫過一遍——那比一句 `SELECT 1` 更能證明「真正要用的那條路通」。所以這個端點先看迴圈剛剛
有沒有跑完，跑完了就直接回答，只有在證據過期（迴圈太久沒有回報成功）的時候才自己問。

這樣一來，監控服務要打多密都不會多花額度，而「有沒有人會被通知」照樣有外部觀測。

（GitHub Actions 那個看門狗不用改：`scripts/watchdog.py` 自己會把參數加上去，所以
`HEALTH_URL` 這個 repository variable 維持原樣就好。）

### `/healthz` 到底檢查什麼

這個網址同時是「保持喚醒」和「唯一的線上監控」。它會實際去測下面這些，任何一項失敗
（帶 `?deep=1` 的時候）就回 **HTTP 503**，UptimeRobot 收到 503 才會寄信給你：

| 檢查項目 | 失敗代表 | 沒帶參數時也會 503？ |
| --- | --- | --- |
| `worker` | 背景 worker 沒在跑迴圈了（卡死，或根本沒起來） | **會**（重開修得好的只有這一個） |
| `database` | 資料庫連不上（服務掛掉、連線字串失效、免費額度用完） | 不會。**沒帶參數時根本不查**（回 `skipped`）；帶了也優先拿盯盤迴圈剛跑完的那一輪當證據，因為每一次自己查都是一次喚醒 |
| `market_data` | 迴圈還在轉，但已經很久沒有一輪報價成功跑完，或連續好幾輪一個價都沒回來 | 不會 |
| `symbols` | 有代號一直抓不到報價——那些代號上的提醒等於停擺，其他不受影響 | 不會 |
| `strategies` | 策略叫不動它的子行程——行情抓得到、迴圈也在轉，但那些策略一支都沒跑 | 不會 |
| `bars` | 抓不到 K 棒——報價回得來，但看 K 線的那幾支策略一支都沒發出提醒 | 不會 |
| `notifications` | 通知被關掉，或最近 24 小時內有提醒重送到放棄還是沒送出去 | 不會 |

「沒帶參數時也會 503」那一欄的規則只有一條：**重開這個行程有沒有機會修好。** 只有
`worker` 有——其他每一個重開一萬次都一樣。理由見上一節。

回應只有檢查項目名稱、秒數和數量，不會帶出連線字串、金鑰或你在看的代號名稱，所以公開
被 ping 是安全的。
...
> 了，被跳過的 job 不計分鐘）。
>
> 代價：CI 的「部署有沒有真的送達」那一步需要這個變數才能確認。那一步只在**你自己推
> 東西上去**的時候才會跑，所以只用這份副本收更新的人不受影響。
...
**要放回去也在同一頁**：風險設定 → 「從備份還原」，選檔案、輸入密碼就好，不用碰資料庫、
不用裝任何東西。它的語意是刻意定成這樣的：

- **只加，不刪也不蓋。** 你當時已經有的策略、持股、風控設定都會原樣留著；備份裡的東西
  是額外加進來的。按錯了只是多出一些東西，而多出來的刪得掉。
- **加回來的策略和通知管道是停用的。** 你自己打開要用的那幾個——不然同一支策略會有兩份
  同時在跑，同一件事通知你兩次。
- **已經持有的部位、已經在自選裡的代號會被跳過**，不會變成兩筆（重複會讓部位和停損算
  錯）。
- **當時還在「待確認」的訊號會標成已過期**：那是好久以前的價格，不該現在再問你一次。
...
1. **先試 app 自己的還原**：風險設定 → 「從備份還原」，選一份 7b 的備份檔。它只加不
   刪，所以就算不確定也可以先試——壞掉的資料不會因此消失，但好的那一份會回來（加回來的
   策略是停用的，你打開要用的那幾個）。
2. Neon 的 **Restore**（免費方案只保留幾小時，過了就沒有）
3. 整個資料庫層級的還原（連同不屬於這個 app 的東西）看第 7c 節的 `pg_restore`
...
- **更新要自己顧。** 後端跟著 `stable` 自動更新，另外放的那一份不會——它是另一個平台上
  ...
  見，但按下更新的是你。
- **自動同步有一種更新推不上去**（#114）。用 Vercel 的部署按鈕建的那一份 GitHub 副本，
  每天會從上游自動同步；但只要那一版上游改到了自動化設定檔（`.github/workflows/` 底下），
  GitHub 就不讓自動同步替你改——那是 GitHub 刻意的安全設計，沒有設定可以打開。這時候
  同步不會推，會在你的副本開一則 issue，寫明兩種做法：
  1. **重新部署一份**（最簡單）：照這一節從頭做一次，把 `VITE_API_BASE_URL` 設回來。
  2. **讓自動更新繼續**：自己建一把有 workflow 權限的 GitHub token，放進副本 repo 的
     Settings → Secrets and variables → Actions，名稱 `SYNC_TOKEN`。這是選擇，不是必要：
     不放的話，只有這種更新要你手動處理。

  畫面和後端放在同一個 Render 服務（本文前面那條路）的人，完全不會遇到這件事。

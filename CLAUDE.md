...
0. **工程標準（不可退讓的底線）。** 每一項都要**真的在跑**，不是寫在文件上。
   ...
   | **DevSecOps** | 安全左移：相依套件漏洞掃描（Dependabot/`pip-audit`）、密鑰不進版控、CI 內做 SAST、部署前檢查設定 | ✅ Dependabot、CI 內 pip-audit、開機檢查密鑰、SAST（`bandit -r app scripts`，CI 內是硬性關卡；抑制一定要附 `# nosec BXXX` 加理由） |
   ...
   | **可觀測性** | Prometheus 指標端點 + Grafana 儀表板：worker 心跳、行情抓取成敗、通知送達率、策略執行延遲。**警告不能停擺，就必須看得到它有沒有在跑** | ⚠️ 外部看門狗 ✅（GitHub Actions 每 15 分鐘打 `/healthz`，掛了寄信；repo variable `HEALTH_URL`）；Prometheus 端點與 Grafana ❌ |

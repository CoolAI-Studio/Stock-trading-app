...
0. **工程標準（不可退讓的底線）。** 每一項都要**真的在跑**，不是寫在文件上。
   ...
   | **Linter** | `ruff check` + `ruff format`（後端）、**`npm run build`**（前端，等於 `tsc -b` + vite build）+ `oxlint`。前端不要只跑 `tsc --noEmit`——它不涵蓋測試檔，漏掉的型別錯誤會等到 CI 的 build 步驟才爆。除錯先看 linter，不要用 print 猜。**圖表相關的改動另外有一關開真的瀏覽器**（`npm run test:chart`，CI 的 `chart` job）：替身和 jsdom 都看不到「選項被吃進去了但行為不如預期」，而那已經讓一個修好的 bug 帶著全綠的 CI 上線過一次 | ✅ |

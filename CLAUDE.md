...
| Neon Postgres | 100 CU-hours／月（官方換算 0.25 CU 跑 400 小時），閒置 5 分鐘休眠且**關不掉** | 每 5 分鐘就碰一次資料庫（收盤後也是），幾乎不休眠 → 需要 730 小時 | 第 17 天前後停到下一個帳單週期 |
| Supabase Postgres | 沒有運算時數計費，只有「paused after 1 week of inactivity」 | 永遠不會閒置一週 | 不會發生 |

由此推出的三條：

1. **資料庫預設推薦 Supabase**，不是 Neon。Neon 留著（設定最快），但它是兩家裡唯一會
   定期讓這個產品停擺的。
2. **保活是必要的**（不保活就是每 15 分鐘睡著、提醒不送），但要跟它的代價一起講：同一
   個帳號下不要再開第二個免費服務。
3. **動到輪詢頻率就是在動這些額度。** `CLOSED_POLL_INTERVAL_SEC` 現在是 300 秒，剛好
   踩在 Neon 的休眠門檻上；要改它是「訊號延遲 vs 免費額度」的取捨，是**要人決定**的事。

...
0. **工程標準（不可退讓的底線）。** 每一項都要**真的在跑**，不是寫在文件上。
   ...
   | **DevSecOps** | 安全左移：相依套件漏洞掃描（Dependabot/`pip-audit`）、密鑰不進版控、CI 內做 SAST、部署前檢查設定 | ✅ Dependabot、CI 內 pip-audit、開機檢查密鑰、SAST（`bandit -r app scripts`，CI 內是硬性關卡；抑制一定要附 `# nosec BXXX` 加理由）、**常態資料外流稽查**（`backend/scripts/audit.py`：CI 每次 push 的硬性關卡 ＋ 每週排程 ＋ 唯讀稽查線上那一份。它不知道這個 app 有什麼——端點從跑起來的 app 讀、資料表從模型登記簿讀——所以新增一個沒有帳號閘門的端點會直接紅燈。做法與三層機制見 `AUDIT.md`）。**線上成品的那一條規則也在推送當下跑**（#110）：bundle 裡帶埠號的 localhost 位址（`audit.LOCALHOST_WITH_PORT`，同一個字串由測試釘住）同時是 CI frontend job 建置後的關卡。`resolveApiBase` 的開發用位址在正式建置裡待了一週才被每週稽查看到——而那時稽查已經因為 react-router 自帶的、不帶埠號的解析基準**每一週都紅**，紅到沒有人看 |

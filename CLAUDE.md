...
- **同步可能不會發生**（Actions 沒開、有衝突、他改過），所以前端把自己的 commit 印
  ...
  打開的 Actions 分頁裡。
- **動到 `.github/workflows/` 的更新，那個同步推不上去**（#114）：`GITHUB_TOKEN` 不能
  改 workflow 檔，沒有任何 `permissions:` 給得了。所以同步先用 `git rev-list` 看要推的
  commit 有沒有碰那個目錄，有就不推，改在他的 repo 開一則 issue（重新部署一份，或自己
  放一把 `SYNC_TOKEN`）。**不可以改回「推推看」**：那是一句英文加一個紅叉，前端從此停
  在舊版。已經在跑的副本拿不到這個改動本身——改它就是動 workflow 檔——所以它們下一次
  遇到時照舊是紅叉，只有之後建的副本受惠；DEPLOYMENT.md〈想把畫面分開放〉寫著。
...
**深的那一條也不自己問**（`health._database_answer`）。`mark_poll_success()` 只在
`tick_once` 整輪沒炸的時候才被呼叫，而那一輪從頭到尾都在讀寫資料庫——所以「剛剛跑完一
輪」是比 `SELECT 1` 更強的證據，而且不用多花一次喚醒。證據過期（超過 `_max_age`）才自
己問。

為什麼一定要做到這一步：那個「每 5 分鐘打 `?deep=1`」的監控是**我們自己的文件教出來
的**，而照做的人不會回來重讀文件，我們也搆不到他的 UptimeRobot。跟資料庫預設那件事是
同一條教訓——**改設定只幫得到還沒裝的人，改程式才救得到已經在跑的那些。**

代價講明白：資料庫在兩輪之間死掉的話，這一格最多晚一個輪詢週期才變紅。沒有比這更早的
辦法——除非讓資料庫永遠不休眠，而那是每個月後半完全停擺。

**`notifications` 那一格也一樣**，而它是漏最久的一個（#100）：它要數「最近有幾則提醒放
棄了」，那是一句 `SELECT count(...)`，就寫在 `SELECT 1` 正下方。#98 拿掉了上面那一句、
#99 擋掉了深層那一次，用量卻兩輪都沒有掉——因為平台的健康檢查每幾秒就打一次沒帶參數的
那一條，而這一句還在那裡。量出來是**每分鐘 15.4 句 SQL**（2026-09-05 21:52–22:07，收盤
後、盯盤迴圈從 35 秒一路睡到 956 秒，`/healthz` 的 `last_sql_age_sec` 一次都沒超過 5
秒）。現在改由迴圈在它那一輪裡數好（`notification/retry.count_recently_given_up`），探
測只讀心跳。

**要新增一格之前先問：它要不要查資料庫？** 要的話，答案只有兩種——由迴圈順便算好，或者
不要加。沒有第三種，因為這支端點被打的頻率是平台決定的，不是我們。

### 貼進來的連線字串要修，但壞掉不要撐（#103）

`DATABASE_URL` 是部署表單上**唯一一格 app 生不出來**的值——只能去別人家的服務複製貼上，
所以它拿到什麼形狀我們控制不了。而貼錯的代價比其他每一格都嚴重：`db/session.py` 在模組
層建引擎，建不起來就是 import 期例外，uvicorn 起不來，**整個網址是死的**。那條路繞過了
`scripts/start.py` 整支的存在理由（跑不動也要起得來、讓設定頁說得出原因）。

所以 `Settings` 有一個 `mode="before"` 的驗證器修三種真的會發生的形狀：`postgres://`
（SQLAlchemy 1.4 拿掉的舊別名，而 Heroku 和大半教學到現在都還在給）、Neon 主控台 psql 分
頁的 `psql '…'`、以及前後的引號和空白。

**但不要為了「撐住」而 fallback 到一個空資料庫。** 一份已經在跑的部署如果連線字串被改
壞，現在是行程死掉 → 平台判部署失敗 → 上一版繼續服務 → 他的提醒沒有斷。改成「起得來但
連到一個空的」反而會讓 `/healthz` 回 200、流量切過去，而那份部署沒有任何資料、worker 也
不跑。**修得動的修，修不動的照樣讓它死**，由
`tests/test_a_pasted_connection_string_still_works.py` 釘著。
...
**而「服務起得來」不等於「遷移跑成了」。** `scripts/start.py` 刻意不用
`alembic upgrade head && uvicorn`：遷移跑不動的時候服務照樣起來（一個死掉的網址送不出任
何說明），已經有帳號的部署更是刻意不鎖。所以這一關真正的斷言不是「curl 得到 200」，而是
**系統狀態頁的 database 那一格**——`start.py` 把原因放進 `DATABASE_MIGRATION_STALE`，那
一格會因此變成 warn 並把原因原樣帶出來。第一版只驗了 curl，那等於沒驗到遷移。
...
每一次健康檢查都在敲資料庫。原因是 `client` fixture 會把 `NOTIFICATIONS_ENABLED` 關掉
（測試不該真的送出通知），而那一格**只有在它開著的時候才會去查資料庫**。

綠燈證明的是另一份程式：一份通知功能關掉的、沒有人在跑的版本。

規則：**一格只在某個設定下才走的路，就要在那個設定下測。** 這條跟「本機有 `.env` 和
`trading_app_dev.db` 而 CI 沒有」是同一種病的兩個方向，而這個 repo 兩邊都被咬過。現在
由 `test_the_shallow_probe_sends_no_sql_with_notifications_switched_on` 釘著——它的名字裡
就寫著那個前提。
...
### 長命的請求不可以坐在一條開著的交易上（#99）

Neon 的規則（compute-lifecycle）：閒置 5 分鐘收起運算單元，**而且會把連線關掉**——所以
單純掛著的連線不是問題。問題是它明講的例外：
...
卡在交易裡的連線一律當成「還在用」，那顆運算單元就**永遠不休眠**。

WebSocket 是這個 app 裡唯一長命的請求，而 `Depends(get_db)` 的 session 要等 handler 回
來才收——所以在修好之前，那條交易的壽命等於使用者的分頁開多久。一個沒關的分頁就足以讓
整個月的額度提前用完，而症狀是後半個月一則提醒都沒有、畫面上什麼都不紅。

規則：**任何活得比一次請求久的東西（WebSocket、SSE、長輪詢），拿到 session 用完就
`close()`，不要等 handler 結束。** 由
`tests/test_an_open_tab_does_not_pin_the_database_awake.py` 釘著。

順帶一提，連線池的 `pool_recycle` 也是這一條的延伸（`db/session.py`）：資料庫真的會睡
了之後，池子裡活得比休眠門檻久的連線一定是死的。
...
同一則提醒會走每一個管道，而每一家收得下多少是它們自己的規則。超過的症狀都一樣：對方回
4xx、重送幾次、那則提醒消失——而那是我們自己送出去的內容造成的。數字來自各家文件，不是猜的：

| 管道 | 上限 | 算的是 | 這裡怎麼守 |
| --- | --- | --- | --- |
| Web Push（iPhone、Android、桌面瀏覽器） | 加密後 4096 bytes（Apple；RFC 8030 §7.2 要求推播服務至少收得下這麼多），aes128gcm 自己佔 103 | **bytes** | `webpush.MAX_PAYLOAD_BYTES`：量整個 JSON 的實際大小 |
| Telegram | 4096 字元 | 字元 | `_fit`，取兩家較小的 4096 |
| LINE | 5000 字元 | 字元 | 同上 |

四條規則：

1. **Web Push 用 bytes 算，不用字元。** 中文一個字 3 bytes，用字元訂的上限不是會超過平台，就
   是浪費一半以上：它曾經是 600 字，收盤摘要（#117）在大約十八檔就被截掉，而實際只用了不到
   一半的額度。現在大約五十檔。
2. **截的時候切在行與行之間，並說出還有幾行。** 半行（半檔股票）比沒有那一行更糟；少了卻不
   說，他會以為那就是全部。
3. **一行本身就超過**（例如策略的例外訊息）才照字元切，結尾加「…」。
4. 鎖定畫面只顯示前幾行、要展開才看得到全部，是作業系統的顯示方式，不是沒送到。**最重要的
   放第一行。**
...
0. **工程標準（不可退讓的底線）。** 每一項都要**真的在跑**，不是寫在文件上。
   ...
   | **DevSecOps** | 安全左移：相依套件漏洞掃描（Dependabot/`pip-audit`）、密鑰不進版控、CI 內做 SAST、部署前檢查設定 | ✅ Dependabot、CI 內 pip-audit、開機檢查密鑰、SAST（`bandit -r app scripts`，CI 內是硬性關卡；抑制一定要附 `# nosec BXXX` 加理由）、**常態資料外流稽查**（`backend/scripts/audit.py`：CI 每次 push 的硬性關卡 ＋ 每週排程 ＋ 唯讀稽查線上那一份。它不知道這個 app 有什麼——端點從跑起來的 app 讀、資料表從模型登記簿讀——所以新增一個沒有帳號閘門的端點會直接紅燈。做法與三層機制見 `AUDIT.md`）。**線上成品的那一條規則也在推送當下跑**（#110）：bundle 裡帶埠號的 localhost 位址（`audit.LOCALHOST_WITH_PORT`，同一個字串由測試釘住）同時是 CI frontend job 建置後的關卡。`resolveApiBase` 的開發用位址在正式建置裡待了一週才被每週稽查看到——而那時稽查已經因為 react-router 自帶的、不帶埠號的解析基準**每一週都紅**，紅到沒有人看。**每週排程其實從 2026-08-24 起就連紅三週**，另一個原因是 pip-audit 抓到 python-ecdsa 一個上游明說不修的公告（PYSEC-2026-1325）——不會有版本可以升，所以它也會永遠紅。解法是換掉帶它進來的 python-jose（→ PyJWT，#111），不是在關卡上加例外：例外要永遠記得理由還成立，而一個每次都紅的關卡等於沒有關卡 |
   ...
   | **可觀測性** | 看得到 worker 心跳、行情抓取成敗、通知送達率。**警告不能停擺，就必須看得到它有沒有在跑** | ✅ 外部看門狗（GitHub Actions 打 `/healthz?deep=1`，掛了寄信。**設定是每 30 分鐘，實際上是好幾個小時一次**（#112）：量到的是 2026-09-05 起間隔中位數 3.6 小時、最長 6.0，而設定成 `*/15` 的那段時間也沒有跑得比較多——GitHub 的排程器是盡力而為。所以它是最後一道網，**不是即時警報**；即時的是 DEPLOYMENT.md 第 4 節那個每 5 分鐘的外部監控。間隔也不再是資料庫預算：#100 之後深的那一條平常讀迴圈的證據、不送 SQL，只有迴圈已經出事時才自己問；repo variable `HEALTH_URL`——**沒設就整個 job 不起來**，見 #96：私人副本上那個排程會把更新用的 Actions 額度吃光）＋ app 內建系統狀態頁（`/system`：worker 心跳、行情抓取成敗、逐代號缺價、通知送達分類）。**Prometheus／Grafana 刻意不做**：指標端點要有人抓才算數，而那需要 Grafana Cloud 帳號和部署表單上的第八個空格，換來一個目標使用者不會打開的儀表板。理由（看得到它有沒有在跑）已由前兩者滿足 |
